            ...
            link: function (scope, element) {
                ...
                scope.getTemplate = function(field){
                    switch (field){
                        ...
                        case "assignee":
                            if(companyUsers){
                                return '<div class="col-md-5 col-sm-5 col-xs-12 form-group has-feedback"> <select class="form-control has-feedback-left" placeholder="Select User" ng-model="ngModel"> <option value="" disabled selected>Select User</option> <option ng-repeat="user in companyUsers" value="{{user.firstname}}"> {{user.firstname}} </option> </select> <span class="fa fa-envelope form-control-feedback left" aria-hidden="true"></span> </div>';
                            }else{
                        ...
                        case "assignee_group":
                            if(companyUserGroups){
                                return '<div class="col-md-5 col-sm-5 col-xs-12 form-group has-feedback"> <select class="form-control has-feedback-left" placeholder="Select User Group" ng-model="ngModel"> <option value="" disabled selected>Select User Group</option> <option ng-repeat="group in companyUserGroups" value="{{group.name}}"> {{group.name}} </option> </select> <span class="fa fa-envelope form-control-feedback left" aria-hidden="true"></span> </div>';
                            }else{
