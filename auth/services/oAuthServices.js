...
(function () {
    ...
    function Service($http, localStorageService, jwtHelper, $auth) {
        ...
        function Logoff(parm, callback) {
            ...
            $http.delete("http://192.168.0.86:3637/oauth/token/revoke/"+decodeToken.jti,  {
                headers: {
        ...
        function Login(parm, callback) {
            $http.post("http://192.168.0.86:3637/oauth/token", {
                grant_type: "password",
        ...
        function getMyPackages(callback) {
            $http.get("http://192.168.0.86:3637/DVP/API/1.0.0.0/MyOrganization/mypackages", {
                headers: {
                    Authorization: 'bearer ' + getToken()
                }
            }).
        ...
        function getAllPackages(callback) {
            $http.get("http://192.168.0.86:3637/DVP/API/1.0.0.0/Packages", {
                headers: {
                    Authorization: 'bearer ' + getToken()
                }
            }).
        ...
        function buyMyPackage(packageName, callback) {
            $http.put("http://192.168.0.86:3637/DVP/API/1.0.0.0/Organisation/Package/" + packageName, {}, {
                headers: {
                    Authorization: 'bearer ' + getToken()
                }
            }).
        ...
        function getUserNavigation(callback) {
            $http.get("http://192.168.0.86:3637/DVP/API/1.0.0.0/MyAppScopes/MyAppScopes/SUPERVISOR_CONSOLE", {
                headers: {
                    Authorization: 'bearer ' + getToken()
                }
            }).
...
//
//
//    var url = "http://192.168.0.86:3637/oauth/token";
